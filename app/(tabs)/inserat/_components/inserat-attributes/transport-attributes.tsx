...
const TransportAttributeRender: React.FC<TransportAttributeRenderProps> = ({ attributes }) => {
    ...
    const attributeList = [
        ...
        { condition: Number(attributes?.doors) !== 0 && attributes?.doors, icon: "door", label: `${attributes.doors}/${Number(attributes.doors) + 1}-Türer`, component: MaterialCommunityIcons },
        { condition: Number(attributes?.weightClass) !== 0 && attributes?.weightClass, icon: "weight", 
        label: `${attributes?.weightClass} kg zulässiges Gesamtgewicht`, component: MaterialCommunityIcons },
